...
import serial
import re
# استيراد مكتبة الكورال لربط مفسر الـ TFLite بالـ Edge TPU هاردوير
...
# ═════════════════════════════════════════════════════════════════════════════
# 3. Model Loading (TFLite ليعمل على الـ Coral Edge TPU)
# ═════════════════════════════════════════════════════════════════════════════
_yolo_session = None
_tflite_interp = None
    ...
    try:
        while True:
            ...
            for bbox, conf, cls_id in zip(boxes, confs, cls_ids):
                # كلاسات اليولو المعتمدة (الأشخاص والسيارات والشاخصات العامة وإشارات المرور)
                ...
                if cls_id == 9: 
                    ...
                    # 🛠️ تم خفض شرط مساحة المربع المقصوص لـ 100 بكسل لتسهيل التجراب المخبرية
                    if (x2-x1)*(y2-y1) > 100:
                        crop = frame[y1:y2, x1:x2]
                        name = classify_sign_crop(crop)
                        # 🔴 طباعة مساعدة لرؤية تصنيف الـ CNN مباشرة بالـ Terminal
                        print(f"🔍 TFLite CNN Classified this crop as: {name}")
                    else:
                        print("⚠️ YOLO saw a sign, but it is too small to crop!")
            ...
            # عرض صندوق الـ CMD والـ FPS الاحترافي على الصورة
