...
# متغيرات المؤقت العالمي لتثبيت الأوامر لمدة 3 ثوانٍ
current_held_cmd = "GO"
last_cmd_time = 0
HOLD_DURATION = 3.0  # مدة التثبيت بالثواني
...
def draw_text_with_shadow(img, text, pos, font, scale, color, thickness=2):
    """دالة مخصصة لكتابة النصوص بظل خلفي ناعم بدون مربعات سوداء غليظة"""
    x, y = pos
...
if __name__ == "__main__":
    ...
    try:
        ...
        # تهيئة موديل يولو الشاخصات المرورية الإضافي
    ...
    try:
        while True:
            ...
            # -----------------------------------------------------------------
            # الموديل الأول: YOLOv8 الأساسي (لكشف العوائق والإشارات الضوئية والشاخصات الأولية)
            # -----------------------------------------------------------------
            try:
            ...
            for bbox, conf, cls_id in zip(boxes, confs, cls_ids):
                ...
                if cls_id == 9: 
                    ...
                    if (x2-x1)*(y2-y1) > 1000:
                        ...
                        # تأكيد الشاخصات المكتشفة بالموديل الأساسي عبر الـ CNN
            ...
            # -----------------------------------------------------------------
            # الموديل الثاني: YOLO الشاخصات الإضافي (42 كلاس) والتحقق الإلزامي عبر الـ CNN
            # -----------------------------------------------------------------
            try:
            ...
            for bbox_s, conf_s, cls_id_s in zip(boxes_s, confs_s, cls_ids_s):
                ...
                # اقتصاص وتمرير الشاخصات المكتشفة بموديل يولو الجديد فوراً إلى الـ CNN لتأكيدها
                crop_s = frame[max(0, y1_s):y2_s, max(0, x1_s):x2_s]
                if crop_s.size > 0:
                    name_s = classify_sign_crop(crop_s)

                    detected_objects_with_info.append({"name": name_s})
                    # رسم الصناديق الخاصة بموديل الشاخصات بلون مختلف (أزرق ناصع) للتمييز البصري
            ...
            draw_text_with_shadow(frame, f"Lock Time: {time_left:.1f}s", (20, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
