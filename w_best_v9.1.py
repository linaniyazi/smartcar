...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # -----------------------------------------------------------------
            # الموديل الثاني: YOLO الشاخصات الإضافي (42 كلاس) الاعتماد المباشر بدون مصفاة
            # -----------------------------------------------------------------
            try:
            ...
            for bbox_s, conf_s, cls_id_s in zip(boxes_s, confs_s, cls_ids_s):
                ...
                # جلب اسم الكلاس مباشرة من قائمة الـ 42 شاخصة بناءً على الـ Index المستخرج من YOLO
                if 0 <= cls_id_s < len(SIGN_CLASSES):
                    name_s = SIGN_CLASSES[cls_id_s]

                    detected_objects_with_info.append({"name": name_s})
                    # رسم الصناديق الخاصة بموديل الشاخصات بلون أزرق ناصع/برتقالي
