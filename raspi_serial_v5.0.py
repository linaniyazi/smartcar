...
def get_decision_v5(detected_list):
    ...
    for obj in detected_list:
        ...
        # 1. تنفيذ أمر STOP
        stop_conditions = [
            "person", "car", "truck", "bus", "stop sign", "Stop", 
        ...
        # 3. تنفيذ أمر SPEED_X (استخلاص الرقم من شاخصات السرعة)
...
class VideoCaptureThread:
    def __init__(self, source=0):
        # تم ضبط المنفذ الافتراضي لكاميرا الـ USB، غيره لـ 1 إذا لم تفتح الكاميرا
...
if __name__ == "__main__":
    ...
    try:
        # ملاحظة: تم إبقاء المنفذ 0 كـ افتراضي لكاميرا الـ USB
    ...
    try:
        while True:
            ...
            for bbox, conf, cls_id in zip(boxes, confs, cls_ids):
                ...
                # فرز وتجهيز البيانات حسب الموديلات
                if cls_id == 9: # إشارة المرور
                    color = get_traffic_light_color(frame, (x1, y1, x2, y2))
                    name = f"TL_{color.upper()}"
                    
                elif cls_id == 11: # إشارة Stop الصريحة من YOLO
                    name = "Stop"
                    
                elif cls_id == 13: # الشاخصات المرورية (السرعات والاتجاهات من TFLite)
                    if (x2-x1)*(y2-y1) > 1000:
                        crop = frame[y1:y2, x1:x2]
                        name = classify_sign_crop(crop)

                # تخزين اسم الكائن المكتشف لإرساله لدالة القرار المحدثة
                detected_objects_with_info.append({
                    "name": name
                })

                # رسم المربع والاسم فوق الجسم
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.putText(frame, name, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                
            # اتخاذ القرار النهائي بناءً على الأسماء الصحيحة التي اخترتها
            final_command = get_decision_v5(detected_objects_with_info)
            arduino.send(final_command)
            
            # عرض الصندوق الأسود لتوضيح الأمر الحالي الموجه للأردوينو على الفيديو
            cv2.rectangle(frame, (15, 15), (260, 55), (0, 0, 0), -1)
