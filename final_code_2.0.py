...
COCO_NAMES = { [cite: 8]
    i: name for i, name in enumerate([ [cite: 8]
        "person","bicycle","car","motorcycle","airplane","bus","train","truck", [cite: 8, 9]
        "boat","traffic light","fire hydrant","stop sign","parking meter","bench", [cite: 9]
...
class VideoCaptureThread: [cite: 9]
    def __init__(self, source=0): [cite: 10]
    ...
    def update(self): [cite: 11]
        while self.started: [cite: 11]
            grabbed, frame = self.cap.read() [cite: 11]
...
def get_traffic_light_color_v8(image, bbox): [cite: 18]
    ...
    gray_roi = cv2.GaussianBlur(gray_roi, (5, 5), 0) [cite: 18, 19]
...
if __name__ == "__main__": [cite: 20]
    ...
    try: [cite: 22]
        while True: [cite: 22]
            ret, frame = cap_thread.read() [cite: 22]
            ...
            try: [cite: 25]
                ...
                boxes, confs, cls_ids = postprocess_yolo( [cite: 25]
                    outputs, scale, pad_left, pad_top, orig_h, orig_w [cite: 26]
                ) [cite: 26]
            ...
            for bbox_s, conf_s, cls_id_s in zip(boxes_s, confs_s, cls_ids_s): [cite: 32]
                ...
                if 0 <= cls_id_s < len(SIGN_CLASSES): [cite: 32, 33]
            ...
            if "SPEED_" in raw_decision: [cite: 35]
            ...
            if current_held_cmd in ["TURN_LEFT", "TURN_RIGHT"] and (current_time - last_cmd_time < TURN_DURATION): [cite: 36]
                # قفل صامت أثناء الالتفاف
                pass  [cite: 36]
            
            else: [cite: 36]
                # إذا خرجنا من مرحلة المنعطف أو كنا في القيادة الطبيعية
                if raw_decision in ["STOP", "SLOW", "TURN_LEFT", "TURN_RIGHT"]: [cite: 37]
                    ...
                    last_cmd_time = current_time  [cite: 38]
            ...
            # إرسال الأمر المستقر عبر السيريال
            arduino.send(current_held_cmd) [cite: 39]
            
            # حساب الوقت المتبقي للمؤقت لعرضه للـ Debugging
            ...
            cv2.imshow(window_name, frame) [cite: 42]
            if out: [cite: 42]
